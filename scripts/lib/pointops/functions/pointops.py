...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True, return_index=False):
    ...
    grouped_xyz = xyz[idx, :].view(m, nsample, 3) # (m, nsample, 3)
    #grouped_xyz = grouping(xyz, idx) # (m, nsample, 3)
    grouped_xyz -= new_xyz.unsqueeze(1) # (m, nsample, 3)
    grouped_feat = feat[idx, :].view(m, nsample, c) # (m, nsample, c)
    #grouped_feat = grouping(feat, idx) # (m, nsample, c)

    if use_xyz:
        grouped_feat = torch.cat((grouped_xyz, grouped_feat), -1) 
    if return_index:
...
def interpolation_flexible(xyz, new_xyz, feat, offset, new_offset, k=3, weight_type='spatial'):
    """
    input: xyz: (m, 3), new_xyz: (n, 3), feat: (m, c), offset: (b), new_offset: (b)
    output: (n, c)
    """
    ...
    if weight_type == 'spatial':
        ...
        scale = (0.3 ** 2) * 2 + 1e-8
        weight = torch.exp(- dist ** 2 / scale) # n, k, 3
