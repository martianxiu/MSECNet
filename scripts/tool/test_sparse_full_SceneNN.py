...
def main():
    ...
    args.save_folder = os.path.join(args.save_folder, "SceneNN")

    test(model,criterion, pred_dim, output_target_ind, output_pred_ind, output_loss_weight) # predict full clouds
    evaluate(args.indir_SceneNN, args.save_folder, args.save_folder) # evaluate sparse patch performance
...
def test(model, criterion, pred_dim, output_target_ind, output_pred_ind, output_loss_weight):
    ...
    data_list = []
    with open(os.path.join(args.indir_SceneNN, 'list/testset_SceneNN.txt')) as f:
...
def evaluate(normal_gt_path, normal_pred_path, output_dir):
    ...
    args.eval_list = ['testset_SceneNN_clean.txt',
                      'testset_SceneNN_noise.txt'] 
    ...
    for cur_list in args.eval_list:
        ...
        for shape in shape_names:
            ...
            normal_gt = np.load(os.path.join(normal_gt_path, 'npy', shape + '.normals.npy'))                  # (n, 3)
