...
class RandomRotate(object):
    ...
    def __call__(self, coord, feat, label):
        ...
        N, C = feat.shape
        if C == 3: 
            feat = np.dot(feat, np.transpose(R)) # rotate normal 
        elif C == 12: # rotate normal and pca normals 
            feat1 = np.dot(feat[:, 0:3], np.transpose(R)) 
            feat2 = np.dot(feat[:, 3:6], np.transpose(R)) 
            feat3 = np.dot(feat[:, 6:9], np.transpose(R)) 
            feat4 = np.dot(feat[:, 9:12], np.transpose(R)) 
...
class RandomPointDrop(object):
    def __init__(self, p=0.2, max_drop_ratio=0.6):
        self.p = p
    ...
    def __call__(self, coord, feat, label):
        if np.random.rand() < self.p:
            N = coord.shape[0]
            drop_ratio = np.random.random() * self.max_drop_ratio
            drop_N = np.floor(N * drop_ratio).astype(np.int)
            # remain the cneter  
            fake_ind = np.arange(1, N)
            remaining_ind = np.random.choice(fake_ind,size=N-drop_N)
            remaining_ind = np.insert(remaining_ind, 0, 0)
            coord = coord[remaining_ind]
        return coord, feat, label
...
class RandomJitter(object):
    def __init__(self, sigma=0.024, clip=0.1, p=0.5):
        self.sigma = sigma
        self.clip = clip
        self.p = p
    def __call__(self, coord, feat, label):
...
class PCARotate(object):
    def __init__(self, return_trans=True):
        super().__init__()
        self.return_trans = return_trans

    def __call__(self, coord, normal, label):
        ...
        # rotate normal 
        normal = torch.matmul(normal, trans)
        ...
        if self.return_trans:
            return coord.numpy(), normal.numpy(), label, trans.numpy() 
        else:
